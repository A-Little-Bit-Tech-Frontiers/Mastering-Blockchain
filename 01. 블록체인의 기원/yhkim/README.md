...
- 비트코인 백서는 해시로 거래를 추적하는 개념(연쇄 서명)을 소개한다.
    - 조폐국 기반 모델에서 정부나 중앙 당국은 거래를 추적하기 위해 표준 회계 관행을 사용 한다.
...
- 비트코인 블록의 주요 속성
    ...
    - 블록 높이 번호(Block height number): 현재 블록과 첫 번째 블록(제네시스 블록) 사이에 얼마나 많은 블록이 있는지 식별한다.
...
- 공개키 암호학과 개인키 암호학
    - 비트코인은 공개키 암호화와 개인키 암호화를 이용해 거래의 타당성을 증명한다.
        - 개인키(비밀키)는 비트코인 거래를 디지털로 서명하는 데 쓰이고, 비트코인 주소의 소유자가 해당 주소의 정당한 소유자임을 증명하는 데 사용된다. 거래를 승인하는 데도 쓰인다.
        - 공개키는 비트코인 주소를 생성하는 데만 사용된다. 비트코인 주소는 누구와도 공개적으로 공유할 수 있다.

- 키 생성
    - 무작위로 선택된 256비트 숫자. 거의 항상 16진수 형식으로 표시돼 있다.
    - 컴퓨터에 의해 생성된다.
    - 개인키는 공개키와 페어링(한쌍으로묶인)돼 비트코인 네트워크에서 거래할수 있다. ing…

- 비트코인 주소 생성
    - 개인키는 ECDSA secp256k1 알고리즘을 사용해 생성
    - 공개키는 SHA256 및 RIPEMD160 해시 함수를 통해 생성
    - 비트코인 주소는 'bc1', '1', '3'으로 시작하며 25~34자로 구성

- 트랜잭션 생성
    - 비트코인의 거래는 UTXO(미사용 거래 산출물) 회계를 따르며, 트랜잭션은 기본적으로 입력 목록과 출력 목록으로 구성된다.

### 조기 취약성

---

초기에는 사용자 수가 적어 프로토콜 문제가 크지 않았으나, **출력 트랜잭션의 이상 현상이 발생하는 보안 결함이 발견되었다**. 2010년 8월 6일, 커뮤니티 회원이 **비정상적으로 많은 양의 출력 거래를 발견**했으며, 개발자 제프 가르직(Jeff Garzik)은 블록 #74638번이 **매우 이상하다고 보고**했다.

이후 취약점이 패치되었으며, **블록체인은 체인을 포크하여 잘못된 거래를 반영하지 않는 방식으로 대응**했다.

### 채택

---

사토시의 실종은 **비트코인을 완전한 탈중앙화 시스템으로 만드는 데 기여**했다.

2010년 5월 22일, **개발자 라즐로 하니에츠(Laszlo Hanyecz)는 피자 두 판을 10,000 BTC에 구매**하며 **비트코인 최초의 실물 거래를 성사**시켰다.

2010년 7월, 암호화폐 거래소 **마운트곡스(Mt. Gox)가 비트코인을 법정통화와 교환할 수 있는 거래소 서비스를 개시**하면서, **비트코인은 본격적인 시장 가치를 가지게 되었다**.
